...
export default async function DashboardHome() {
  ...
  const [
    ...
    supabase.from('notifications').select('*', { count: 'exact', head: true }).eq('roofer_id', user!.id).is('dismissed_at', null).eq('type', 'call_needed'),
                ...
                <div key={b.id} className="bg-zinc-900 rounded-xl border border-green-500/30 p-5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <a
                        href={`/api/calendar?booking=${b.id}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 text-sm font-semibold px-4 py-2 rounded-lg transition-colors"
                      >
                      ...
                      <a
                        ...
                        className="flex items-center gap-2 bg-green-500/10 hover:bg-green-500/20 text-green-400 text-sm font-semibold px-4 py-2 rounded-lg transition-colors"
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
