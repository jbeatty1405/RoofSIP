  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 mb-8">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="font-semibold text-zinc-200 text-sm">Get set up ({completed}/{steps.length} done)</p>
          <div className="mt-1.5 h-1.5 w-48 bg-zinc-800 rounded-full">
            <div
        ...
        {steps.map((step, i) => (
          <Link
            ...
            className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${
              step.done ? 'opacity-40 cursor-default' : 'hover:bg-zinc-800 cursor-pointer'
            }`}
          >
            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
              step.done ? 'bg-sky-500 border-sky-500' : 'border-zinc-600'
            }`}>
              {step.done && (
            ...
            <div className="flex-1">
              <p className={`text-sm font-medium ${step.done ? 'line-through text-zinc-600' : 'text-zinc-200'}`}>{step.label}</p>
